...
func runFFmpegWithProgress(cmd *exec.Cmd, totalDuration float64, onProgress func(percent int)) error {
	...
	emitProgress := func(currentSec float64) {
	...
	stderrScanner := newScanner(stderr)
	stdoutScanner := newScanner(stdout)
	...
	go func() {
		...
		for stderrScanner.Scan() {
			...
			if currentSec, ok := ffmpegStatusSeconds(line); ok {
				emitProgress(currentSec)
			}
		}
	}()
	go func() {
		...
		for stdoutScanner.Scan() {
			...
			if currentSec, ok := ffmpegProgressSeconds(line); ok {
				emitProgress(currentSec)
				continue
			}
			if line == "progress=end" {
				emitProgress(totalDuration)
			}
		}
	}()
...
func ffmpegStatusSeconds(line string) (float64, bool) {
...
// ffmpeg -progress emits key=value records on stdout. Despite its historical
// name, out_time_ms is expressed in microseconds, the same as out_time_us.
func ffmpegProgressSeconds(line string) (float64, bool) {
