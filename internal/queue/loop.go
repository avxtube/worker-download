...
func RunLoop(ctx context.Context, workerID string, handler JobHandler) {
	...
	for {
		...
		if total, used, _ := getDiskUsage(config.AppConfig.WorkDir); total > 0 {
		...
		if !workerEnabled(ctx, workerID) {
			...
			err := Release(releaseCtx, job.ID, workerID)
			cancel()
			if err != nil {
...
func watchCancel(jobCtx context.Context, cancelJob context.CancelCauseFunc, jobID, workerID string) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err == nil && p.Status != nil && *p.Status == enums.ProcessStatusProcessing &&
				p.WorkerID != nil && *p.WorkerID == workerID {
				now := time.Now()
				_, _ = models.VideoProcessModel.Col().UpdateOne(qCtx,
					bson.M{"_id": jobID, "status": enums.ProcessStatusProcessing, "workerId": workerID},
					bson.M{"$set": bson.M{"heartbeatAt": now, "leaseExpiresAt": now.Add(2 * time.Minute), "updatedAt": now}},
				)
			}
			cancel()
...
func runJob(ctx context.Context, workerID string, job *models.VideoProcess, handler JobHandler) {
	...
	defer cancelJob(nil)
	go watchCancel(jobCtx, cancelJob, job.ID, workerID)
	...
	switch {
	case err == nil:
		if e := Complete(settleCtx, job.ID, workerID); e != nil {
